...
class CurlClient implements ClientInterface
{
    ...
    {
        if ($numRetries >= MomoApi::getMaxNetworkRetries()) {
            return false;
        }
    ...
    private function sleepTime($numRetries)
    {
        ...
        $sleepSeconds = min(
            MomoApi::getInitialNetworkRetryDelay() * 1.0 * pow(2, $numRetries - 1),
            MomoApi::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = max(MomoApi::getInitialNetworkRetryDelay(), $sleepSeconds);
