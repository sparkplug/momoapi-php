...
class CurlClientTest extends TestCase
{
    ...
    {
        $this->origMaxNetworkRetries = MomoApi::getMaxNetworkRetries();
        $this->origMaxNetworkRetryDelay = MomoApi::getMaxNetworkRetryDelay();
        $this->origInitialNetworkRetryDelay = MomoApi::getInitialNetworkRetryDelay();
    }
    ...
    {
        $stripeReflector = new \ReflectionClass('\MomoApi\MomoApi');
        ...
        $curlClientReflector = new \ReflectionClass('MomoApi\HttpClient\CurlClient');
    ...
    {
        MomoApi::setMaxNetworkRetries($this->origMaxNetworkRetries);
    ...
    {
        $fakeRandomGenerator = $this->getMock('MomoApi\Util\RandomGenetator', ['randFloat']);
    ...
    public function testSleepTimeShouldGrowExponentially()
    {
        ...
        $this->assertEquals(
            MomoApi::getInitialNetworkRetryDelay() * 1,
            $this->sleepTimeMethod->invoke($curlClient, 1)
        );
        $this->assertEquals(
            MomoApi::getInitialNetworkRetryDelay() * 2,
            $this->sleepTimeMethod->invoke($curlClient, 2)
        );
        $this->assertEquals(
            MomoApi::getInitialNetworkRetryDelay() * 4,
            $this->sleepTimeMethod->invoke($curlClient, 3)
        );
        $this->assertEquals(
            MomoApi::getInitialNetworkRetryDelay() * 8,
    ...
    public function testSleepTimeShouldAddSomeRandomness()
    {
        ...
        $baseValue = MomoApi::getInitialNetworkRetryDelay() * (0.5 * (1 + $randomValue));

        // the initial value cannot be smaller than the base,
        // so the randomness is ignored
        $this->assertEquals(MomoApi::getInitialNetworkRetryDelay(), $this->sleepTimeMethod->invoke($curlClient, 1));
